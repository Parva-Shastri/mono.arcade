...
registerTest({
  ...
  run: async () => {
    ...
    if (!content.includes('State: 5')) {
      throw new Error(`Intermediate score/state not updated to 5, body content: ${content}`);
    }

    const scores = JSON.parse(localStorage.getItem('mono_games_scores') || '{}');
    const record = scores[gameId];
    if (!record || record.gamesPlayed !== 1 || record.gamesWon !== 1 || record.highScore !== 950) {
      throw new Error(`Expected record to show 1 played, 1 won, 950 highScore. Got: ${JSON.stringify(record)}`);
    }
  }
});
...
registerTest({
  ...
  run: async () => {
    ...
    if (!content.includes('State: 5')) {
      throw new Error(`Expected State: 5 before resetting, got body: ${content}`);
    }
