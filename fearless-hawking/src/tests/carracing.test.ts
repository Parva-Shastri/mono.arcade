...
const gameId = 'carracing';
const gameTitle = 'Car Racing';
...
registerTest({
  ...
  run: async () => {
    ...
    if (!content.includes('State: 10')) {
      throw new Error(`Intermediate score/state not updated to 10, body content: ${content}`);
    }

    const scores = JSON.parse(localStorage.getItem('mono_games_scores') || '{}');
    const record = scores[gameId];
    if (!record || record.gamesPlayed !== 1 || record.gamesWon !== 1 || record.highScore !== 100) {
      throw new Error(`Expected record to show 1 played, 1 won, 100 highScore. Got: ${JSON.stringify(record)}`);
    }
  }
});
...
registerTest({
  ...
  run: async () => {
    ...
    if (!content.includes('State: 10')) {
      throw new Error(`Expected State: 10 before resetting, got body: ${content}`);
    }
    ...
    if (!content.includes('State: 0')) {
      throw new Error(`Stub state did not reset to 0, body content: ${content}`);
    }
  }
});
